...
export function useStoragePanel() {
	...
	if (!ctx) throw new Error("useStoragePanel must be used within StoragePanelProvider");
	return ctx;
}

export function StoragePanelProvider({ children }: { children: React.ReactNode }) {
	const value = useStoragePanelState();
	return <StoragePanelContext.Provider value={value}>{children}</StoragePanelContext.Provider>;
}
...
function useStoragePanelState() {
	...
	const getTokenWithRenew = useCallback(async () => {
		try {
			...
			if (msg.includes("Missing Refresh Token") || msg.includes("login_required") || msg.includes("consent_required")) {
	...
	const refresh = useCallback(
		async (path?: string) => {
			...
			if (shareRes.ok) {
				...
				((shareRes.data as any).shares || []).forEach((s: ShareResult) => (map[s.key] = s));
				share.setOwnedShares(map);
			}
		},
		[api, pushToast, share]
	);
	...
	const uploadFiles = useCallback((files: File[], targetPath?: string) => uploads.uploadFiles(files, targetPath), [uploads]);
	const drag = useDragAndMove({ api, pushToast, uploadFiles, refresh, currentPath });
	...
	const versions = useVersions({ api, pushToast, refresh: () => refresh(currentPath) });
	...
	const copyText = useCallback(
		async (text: string, label = "Copied") => {
			try {
				...
				pushToast("error", "Could not copy", err instanceof Error ? err.message : String(err));
			}
		},
		[pushToast]
	);
	...
	const onNewFolder = useCallback(async () => {
		...
		const existing = new Set([...(list?.directories || []).map((d) => d.name), ...(list?.files || []).map((f) => f.name)]);
		let name = base;
	...
	const onDownloadFolderZip = useCallback(
		async (path: string) => {
			try {
				...
				const base = String((import.meta as any).env.VITE_API_BASE || "http://localhost:8000").replace(/\/$/, "");
				const url = `${base}/storage/download-zip?path=${encodeURIComponent(path)}`;

				const res = await fetch(url, {
					method: "GET",
					headers: { Authorization: `Bearer ${token}`, ...(idToken ? { "X-Auth0-Id-Token": idToken } : {}) },
				});

				if (!res.ok) return void pushToast("error", "Download failed", (await res.text()) || `HTTP ${res.status}`);
...
function useVersions({
	...
	api: any;
	pushToast: (variant: Toast["variant"], text: string, details?: string) => void;
