      ...
      {uploadTasks.length === 0 ? (
        ...
        <div className="grid gap-2">
          {uploadTasks.map((t) => {
            ...
            const pct = total > 0 ? Math.max(0, Math.min(100, (uploaded / total) * 100)) : 0;

            const hashed = Math.max(0, Math.min(hashedRaw, total || hashedRaw));
            const hashPct = total > 0 ? Math.max(0, Math.min(100, (hashed / total) * 100)) : 0;

            return (
              <div key={t.id} className="rounded-md border bg-background/40 px-3 py-2">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {t.targetPath ? (
                      <div className="mt-0.5 text-xs text-muted-foreground">Destination: {t.targetPath}</div>
                    ) : null}
                  </div>
                  <div className="flex items-center gap-2">
                    {t.status === "finalizing" ? (
                      ...
                      <Button variant="outline" size="sm" onClick={() => uploads.pauseUpload(t.id)}>
                        Pause
                      </Button>
                    ) : (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => uploads.resumeUpload(t, () => fileRef.current?.click())}
                      >
                        Resume
                      </Button>
                    )}
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        void uploads.cancelUpload(t.id, t.fileChecksum || undefined);
                      }}
                    >
                ...
                {t.status === "error" && (
                  <div className="mt-2 text-xs text-destructive">Upload failed: {t.error || "Unknown error"}</div>
                )}

                {t.status !== "error" && t.note && (
                  <div className="mt-2 text-xs text-muted-foreground">{t.note}</div>
                )}
              </div>
            );
          })}
